...
```sh
npm run release:workflows -- v1.0.0
```

Create local release tags after verification:

```sh
npm run release:workflows -- v1.0.0 --execute
```

Create and push the tags:

```sh
npm run release:workflows -- v1.0.0 --execute --push
```

The script creates an immutable annotated tag, such as `v1.0.0`, and updates the matching floating major tag, such as `v1`.
